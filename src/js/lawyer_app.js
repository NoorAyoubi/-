// Lawyer Dashboard Entrypoint Module
import { translations, getCurrentLang, setCurrentLang, updateDirectionAndLang } from '../modules/language.js';
import { getSubmissions, saveSubmissions, clearSubmissions } from '../modules/storage.js';
import { startCelebration } from '../modules/celebration.js';
import { triggerDesktopNotification, playChime } from '../modules/notification.js';

// DOM Selectors
let leadsTableBody, archiveTableBody, detailModal, closeModalBtn, modalDetailsBody;
let btnLeads, btnArchive, btnSettings, leadsSection, archiveSection, settingsSection;
...
function translateLocation(loc, lang) {
    ...
    if (loc.includes('إسرائيلية') || loc.includes('Israeli') || loc.includes('ישראلي')) {
    ...
    if (!isFirstLoad) {
        submissions.forEach(lead => {
            if (!knownSubmissionIds.has(lead.id)) {
                triggerDesktopNotification(lead, (clickedId) => {
                    window.focus();
                    showLeadDetails(clickedId);
                });
            }
        });
    }
...
function renderTable(submissions) {
    ...
    const t = translations[getCurrentLang()].dashboard;
    if (!t) return;
    ...
    const leadsBtn = document.getElementById('btnLeads');
    if (leadsBtn) {
...
function toggleProcessed(id, isChecked) {
    let submissions = getSubmissions();
    ...
    if (leadIndex > -1) {
        ...
        if (isChecked) {
            ...
            const msg = lang === 'en' ? "✅ Task completed successfully." : (lang === 'he' ? "✅ המשימה הושלמה בהצלחה." : "✅ تم إنجاز المهمة بنجاح.");
...
function toggleModalTranslation() {
...
window.toggleModalTranslation = toggleModalTranslation;
...
function renderModalContent(id) {
    ...
    const renderLang = isModalTranslated ? lang : clientLang;
    const tSub = translations[renderLang].dashboard || translations['ar'].dashboard;
    
    const details = lead.accidentDetails || "---";
    const otherParty = lead.isNegligence || "---";
    const locationBefore = lead.locationBefore || "---";
    ...
    if (clientLang !== lang) {
        ...
        bannerHtml = `
            <div id="translationBanner" style="background: rgba(197, 168, 128, 0.06); border: 1px dashed var(--accent-gold); border-radius: 12px; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; direction: ${lang === 'en' ? 'ltr' : 'rtl'};">
    ...
    modalDetailsBody.innerHTML = `
        ...
        <div style="display: flex; flex-direction: column; gap: 14px; line-height: 1.6; font-size: 0.95rem;">
            ...
            <div>
                <strong>${t.mPhone}</strong> <span><a href="tel:${lead.clientPhone}" style="color: var(--accent-gold); text-decoration: none;">${lead.clientPhone}</a></span>
            </div>
            <div>
            ...
            <div style="background-color: rgba(251, 191, 36, 0.02); border: 1px solid rgba(251, 191, 36, 0.15); border-radius: 8px; padding: 12px;">
...
function resetDatabase() {
    ...
    if (confirm(t.resetConfirm)) {
        ...
        const msg = lang === 'en' ? "🧹 Database wiped successfully." : (lang === 'he' ? "🧹 בסיס הנתונים אופס בהצלחה." : "🧹 تم تفريغ قاعدة البيانات بنجاح.");
        startCelebration(msg);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    leadsTableBody = document.querySelector('#leadsSection tbody');
    archiveTableBody = document.querySelector('#archiveSection tbody');
